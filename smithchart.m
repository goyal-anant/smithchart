function yzsmith(r,x,N)
    %For Drawing YZ smith chart
    
    %N = no of points for circular variation, understand it by say number of values
    %of x in f(x), r and x are vectors for getting values of constant 'r'
    %and 'x' circles
    
    %% Z-Smith Chart
    
    %plotting r = 0 i.e. outermost circle of smith chart
    plot(cos(linspace(0,2*pi,N)),sin(linspace(0,2*pi,N)),'b','LineWidth',1.5);
    hold on;
